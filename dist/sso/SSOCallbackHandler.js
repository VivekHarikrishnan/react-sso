...
class SSOCallbackHandler extends React.Component {
  async componentDidMount() {
    ...
    let queryParams = qs.parse(window.location.search.replace("?", "")); // When query params has code.

    let is_valid = new Boolean(queryParams.code).valueOf();
    let authResponse = {};

    if (is_valid) {
      authResponse = await SSOService.invokeTokenEndpoint(this.props.token_endpoint_url, queryParams.code, this.props.config, this.props.auth_header);
      is_valid = authResponse.status === 200;
    }

    if (is_valid && authResponse.data) {
      // Set user session variables and allow user to sign-in.
      this.props.sessionHandler(authResponse.data); // Update the redirect URL depends on the app.
  ...
  render() {
    ...
    return /*#__PURE__*/React.createElement(WrapperComponent, null);
  }

}

SSOCallbackHandler.defaultProps = {
  WrapperComponent: /*#__PURE__*/React.createElement("div", null, "Please wait while we authorize you!")
};
